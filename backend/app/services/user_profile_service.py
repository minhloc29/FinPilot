...
class UserProfileService:
    ...
    def create_or_update_profile(
        ...
        try:
            ...
            profile = UserProfileService.get_user_profile(db=db, user_id=user_id)
            if not profile:
            ...
            risk_keys = {"risk_profile", "max_drawdown_tolerance", "investment_horizon_years"}
            ...
            if any(key in payload for key in risk_keys):
                if not risk:
                    risk = RiskProfile(
                        ...
                        max_drawdown=payload.get("max_drawdown_tolerance") or 20.0,
                        investment_horizon_years=payload.get("investment_horizon_years") or 5,
                    )
                    db.add(risk)
                else:
            ...
            db.rollback()
            logger.error("Error updating onboarding profile for user_id=%s: %s", user_id, str(exc))
            raise
    ...
    def delete_profile(db: Session, user_id: int) -> bool:
        try:
            ...
            db.rollback()
            logger.error("Error deleting profile for user_id=%s: %s", user_id, str(exc))
            raise
